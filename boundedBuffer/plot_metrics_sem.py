...
MAIN_BIN = os.path.join(SCRIPT_DIR, "main_sem")
...
def build():
    cmd = [
        "gcc", "-o", "main_sem",
        "main.c", "boundedBuffer_semaphore.c", "producer_consumer.c", "tasks.c", "task.c",
        "-lpthread"
    ]
...
def run_benchmark(num_threads):
    ...
    if result.returncode != 0:
        raise RuntimeError(f"main_sem exited {result.returncode}\n{result.stderr}")
...
def main():
    print("Building main_sem (Semaphore)...", flush=True)
    build()
    ...
    for ax, (key, ylabel, title) in zip(axes, titles):
        ax.plot(THREAD_COUNTS, data[key], marker="s", linewidth=2, markersize=8, color="#e74c3c")
    ...
    plt.suptitle("Bounded Buffer — Semaphore", fontsize=13, y=1.02)
    plt.tight_layout()
    out_path = os.path.join(GRAPHS_DIR, "metrics_plot_sem.png")
